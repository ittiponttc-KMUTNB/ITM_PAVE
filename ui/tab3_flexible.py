...
# ─────────────────────────────────────────────
#  Badge helper (style เดียวกับ Tab 4)
# ─────────────────────────────────────────────
...
def render():
    ...
    with col_fl:
        ...
        with st.container(border=True):
            st.markdown('<div class="rp-card-title">📥 Design ESAL</div>',
        ...
        # ── Subgrade ──
        with st.container(border=True):
            st.markdown('<div class="rp-card-title">🌍 Subgrade</div>',
                        unsafe_allow_html=True)
            ...
            # ── Reference panel — แสดงข้อมูลอย่างเดียว ──
            _ref_parts = []
            if _cbr1:
                _ref_parts.append(
        ...
        # ── Design Parameters ──
        with st.container(border=True):
            st.markdown('<div class="rp-card-title">⚙️ Design Parameters</div>',
                        unsafe_allow_html=True)
            ...
            with c1:
                ...
                zr_fl = ZR_MAP[r0_fl]
                st.markdown(
                    _badge('ZR', f'{zr_fl:.3f}', '', bg='#EEF2F7', color='#546E7A'),
                    unsafe_allow_html=True
                )
            with c2:
    ...
    with col_fr:
        with st.container(border=True):
            st.markdown('<div class="rp-card-title">🔩 Layer Design</div>',
                        unsafe_allow_html=True)
...
def _render_layer_result(st_obj, li, mat_f, h_f, ai, mi_f, mr_psi_layer,
    ...
    if _badge_html:
        st_obj.markdown(_badge_html, unsafe_allow_html=True)
